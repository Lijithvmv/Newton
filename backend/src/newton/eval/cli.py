...
from .tasks import HARD_TASKS, LARGE_TASKS, SEED_TASKS
...
def main() -> None:
    ...
    ap.add_argument("--suite", default="seed", choices=["seed", "hard", "large"], help="task suite")
    ...
    print(f"Running {len(tasks)} task(s) × {args.reps} rep(s) × {len(arms)} arm(s) on {args.model} …\n")
